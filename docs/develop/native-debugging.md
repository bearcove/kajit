# Native debugging with LLDB and GDB

When a bug doesn't reproduce in the [MIR interpreter](mir-debugger.md), it's a
native codegen or ABI issue — something in register allocation, calling
conventions, or the machine code itself. This page covers how to attach a
debugger to JIT-compiled code on macOS and Linux.

kajit registers generated code with the GDB JIT interface. This means LLDB and
GDB can resolve JIT symbol names, but only through explicit lookup — `thread
backtrace` may still show raw PCs for top JIT frames because kajit emits a
minimal JIT ELF (`.text` + symbol table) without full DWARF/unwind metadata.
Use `image lookup` (LLDB) or `info symbol` (GDB) instead of relying on
backtraces.
...
```gdb
stepi   # repeat until you land in kajit::decode::*
...
- Explicit symbol lookup (`image lookup` in LLDB, `info symbol` in GDB) is more
  reliable.
- Current JIT symbolization is enough for function-level resolution, not full

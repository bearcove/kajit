...
use kajit_ir::{
    DebugScopeId, DebugValueId, IrFunc, IrOp, LambdaId, Node, NodeId, NodeKind, OutputUseKind,
    PortKind, PortSource, RegionId, VReg,
};

struct Linearizer<'a> {
    /// The IR function currently being linearized.
    func: &'a IrFunc,

    /// Emitted flat linear operations.
    ops: Vec<LinearOp>,

    /// Counter used to allocate fresh labels.
    label_count: u32,

    /// Debug scopes corresponding to emitted operations.
    op_scopes: Vec<Option<DebugScopeId>>,

    /// Debug values corresponding to emitted operations.
    op_values: Vec<Option<DebugValueId>>,

    /// Debug scopes recorded for each vreg.
    vreg_scopes: Vec<Option<DebugScopeId>>,

    /// Debug values recorded for each vreg.
...
fn op_defs(op: &LinearOp) -> Vec<VReg> {
    match op {
        ...
        LinearOp::StackAlloc { dst, .. } => vec![*dst],
        LinearOp::LoadFromAddr { dst, .. } => vec![*dst],
        LinearOp::Call { dst, .. } => vec![*dst],
...
fn rewrite_op_uses(op: &mut LinearOp, mut resolve: impl FnMut(VReg) -> VReg) {
    ...
    match op {
        ...
        LinearOp::Call { args, .. } | LinearOp::CallLambda { args, .. } => {
            for arg in args {

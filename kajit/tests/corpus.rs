    ...
    F: kajit::format::Decoder,
{
    ...
    if should_assert {
        insta::assert_snapshot!(
            format!("generated_rvsdg_{}_{}_{}", format_label, case,
            std::env::consts::ARCH), artifacts.ir_text
        );
    }
}
    ...
    F: kajit::format::Decoder,
{
    ...
    if should_assert {
        insta::assert_snapshot!(
            format!("generated_ra_mir_{}_{}_{}", format_label, case,
            std::env::consts::ARCH), artifacts.ra_text
        );
    }
}
    ...
    F: kajit::format::Decoder,
{
    ...
    if should_assert {
        insta::assert_snapshot!(
            ...
            std::env::consts::ARCH), format!("{}", artifacts.edits)
        );
    }
}
mod json {
...
mod postreg {
    ...
    fn vec_scalar_large_hotpath_asserts() {
        let artifacts = codegen_artifacts::<
            ScalarVec,
            _,
        >(&kajit::postcard::KajitPostcard);
        assert!(
        ...
        assert!(
            artifacts.ra_text.contains("branch_if"), "expected loop backedge in RA-MIR"
        );
        assert!(
        ...
        assert!(
            artifacts.edits <= 128, "expected edit budget <= 128, got {}", artifacts
            .edits
        );
    }
}

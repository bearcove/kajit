...
mod format_specific {
    ...
    fn postcard_long_varints_via_ir() {
        ...
        let via_ir = kajit::compile_decoder_via_ir(
            Person::SHAPE,
            &kajit::postcard::KajitPostcard,
        );
    ...
    fn json_struct_ir_orders_key_read_before_key_compare() {
        let linear_text = kajit::debug_linear_ir_text(
            Friend::SHAPE,
            &kajit::json::KajitJson,
        );
        let read_key_pat = format!(
            "call_intrinsic 0x{:x}", kajit::json_intrinsics::kajit_json_read_key as *
            const () as usize
        );
        let key_eq_pat = format!(
            "call_pure 0x{:x}", kajit::json_intrinsics::kajit_json_key_equals as * const
            () as usize
        );
        ...
        let read_key_pos = linear_text
            .find(&read_key_pat)
            .unwrap_or_else(|| {
                panic!("missing read_key call in linear IR:\n{linear_text}")
            });
        ...
        let key_eq_pos = linear_text
            .find(&key_eq_pat)
            .unwrap_or_else(|| {
                panic!("missing key_equals call in linear IR:\n{linear_text}")
            });
        assert!(
    ...
    fn postcard_borrowed_str_zero_copy() {
        let input = [0x2A, 0x05, b'A', b'l', b'i', b'c', b'e'];
        let deser = kajit::compile_decoder(
            BorrowedFriend::SHAPE,
            &kajit::postcard::KajitPostcard,
        );
    ...
    fn json_f64_edge_cases() {
        ...
        for (input, expected) in cases {
            ...
            assert_eq!(
                result.x.to_bits(), expected.to_bits(),
                "input={:?}: got {} ({:#018x}), expected {} ({:#018x})",
                std::str::from_utf8(input).unwrap(), result.x, result.x.to_bits(),
            ...
            if json_bytes[i] == b'-' || json_bytes[i].is_ascii_digit() {
                ...
                if s.contains(&b'.') || s.contains(&b'e') || s.contains(&b'E') {
                    ...
                    let json_input = format!(
                        r#"{{"v":{}}}"#, std::str::from_utf8(s).unwrap()
                    );
                    let result: Coord = kajit::deserialize(&deser, json_input.as_bytes())
                        .unwrap();
                    if std_val.to_bits() != result.v.to_bits() {
                        if mismatches < 10 {
                            eprintln!(
                                "MISMATCH: {:?} -> std={std_val:?} jit={:?}",
                                std::str::from_utf8(s).unwrap(), result.v,
                            );
                        }
                        mismatches += 1;
                    }
    ...
    fn postcard_vec_u32_medium_large_ir_matches_legacy_and_serde() {
        ...
        let ir = kajit::compile_decoder_via_ir(
            Nums::SHAPE,
            &kajit::postcard::KajitPostcard,
        );
